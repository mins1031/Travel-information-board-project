 ...
 <nav class="navbar fixed-top navbar-expand-lg navbar-dark bg-dark fixed-top">
   ...
   <div class="container">
      <div class="row">
        ...
        <div class="col-lg-12">
          <h2 class="page-header"><sec:authentication property="principal.username"/>의 게시판</h2>
        </div>
      
      <table class="table table-striped table-bordered table-hover">
       <thead>
          <tr><th>번호</th><th>제목</th><th>작성자</th><th>작성일자</th><th>조회수</th><th>게시판 타입</th></tr>
       </thead>
       
         <c:forEach items="${boardList}" var="board">
         <tr class="board">
           <td><c:out value="${board.bno}" /></td>
           <td><a class='move' value="<c:out value="${board.region.regionId}"/>" href='<c:out value="${board.bno}"/>'>
           ...
           <td><c:out value="${board.region.regionId}" /></td>
      ...
      <!-- 페이징 끝 -->
       ...
       <form action="/user/userBoard" id="actionForm" method="get">
    	  ...
    	  var actionForm = $("#actionForm");
           ...
           //페이징 된 번호들 클릭시 해당 페이지로 이동시키는 로직           
       	  ...
       	  $(".move").on('click',function(e){
       		  ...
       		  var boardType = $(this).attr("value");
       		  //클릭된 a태그내의 value값 저장
       		  
       		  actionForm.append("<input type='hidden' name='bno' value='"+ $(this).attr("href") + "'>");
       		  actionForm.append("<input type='hidden' name='userInfoToken' value='userToken'>"); 
       		  actionForm.find("input[name='boardType']").val(boardType);//bno의 값대로 href값이 정해짐.
