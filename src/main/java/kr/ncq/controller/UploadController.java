...
import kr.ncq.domain.AttachFileDTO;
	...
	public ResponseEntity<List<AttachFileDTO>> uploadAjaxPost(MultipartFile[] uploadFile) {
		...
		for (MultipartFile multipartFile : uploadFile) {
			...
			try {
				...
				attacDTO.setUuid(uuid.toString());
				attacDTO.setUploadPath(uploadFolderPath);
				
				if(checkImageType(saveFile)) {
					
					attacDTO.setImage(true);
	...
	@GetMapping("/display")
	  ...
	  public ResponseEntity<byte[]> getFile(String fileName) {
		  ...
		  try {
			  ...
			  //http 바디에 헤더내용, bytearray로 바꾼 file,ok메세지를 담아 보내야 하기에 담아주는 역할인듯
		  } catch(IOException e) {
			  e.printStackTrace();
		  }
		  return result;
	  ...
	  public ResponseEntity<Resource> downloadFile(@RequestHeader("User-Agent") String userAgent,
		  ...
		  // 그부분부터 출력하면 " _파일진짜이름" 이 되기 떄문에 +1을 한것. 이걸로 파일 다운로드되는 부분에도 파일네임만이 들어감.
